    ...
    public function insertData($catId,$title,$content,$uId)
        ...
        public function selectCategories()
        {
            ...
            if($res->num_rows > 0)
            {
                while($row=$res->fetch_assoc())
                {
                    echo "<option><a href=\"categoriesPost.php?id={$row["id"]}\">{$row['cat_name']}</option>";          
                }
            }
        }
        ...
        {
            $sql = "SELECT posts.id,posts.title,posts.content, posts.cat_id FROM posts 
			JOIN categories ON posts.cat_id=categories.id ORDER BY title DESC";            
            $res = $this->db->query($sql);          
            if($res->num_rows > 0)
            {
                while($row=$res->fetch_assoc())
                {  
                    echo "<a href=\"viewPostDetails?id={$row['id']}\"><h1>{$row['title']}</h1></a>";
